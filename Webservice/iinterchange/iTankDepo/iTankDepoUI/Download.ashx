...
Public Class Download : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        Dim strExtn As String = IO.Path.GetExtension(strFile).ToLower()
        If strInvc_Type = "Received" Then
            strFileName = String.Concat(CommonWeb.pub_GetConfigValue("Received").ToString, strFile)
        ElseIf strInvc_Type <> "" And (strExtn = ".xml" Or strExtn = "xml") Then
            strFileName = String.Concat(CommonWeb.pub_GetConfigValue("XMLEDI").ToString, strInvc_Type, "\", strFile)
        ElseIf strInvc_Type <> "" Then
            strFileName = String.Concat(CommonWeb.pub_GetConfigValue("UploadPhyPath").ToString(), "Invoice\", strInvc_Type, "\", strFile)
            
        Else
            strFileName = String.Concat(CommonWeb.pub_GetConfigValue("UploadPhyPath").ToString(), strFile)
        End If
