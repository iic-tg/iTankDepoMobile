...
#End Region
    ...
    Protected Overrides Sub OnStart(ByVal args() As String)
        ...
        'Me.tmrDAR.Interval = CDbl(Config.pub_GetAppConfigValue("DARTimerInterval"))
                ...
                SyncLock tmrDAR
                    ...
                    If dtScheduleTime.Rows.Count > 0 Then
                        ...
                        For Each drScheduleTime As DataRow In dtScheduleTime.Rows
                            ''Added by Arun starts here 02/02/2016
                            Dim drCheckRecords() As DataRow = dsEmailSetting.Tables(DARData._V_CUSTOMER_EMAIL_SETTING).Select(DARData.CSTMR_ID & "=" & drScheduleTime.Item(DARData.CSTMR_ID))
                            If drCheckRecords.Length = 0 Then
                                If Not IsDBNull(drScheduleTime.Item("PRDC_DT_ID")) Then
                                    i32PRDC_DT = drScheduleTime.Item("PRDC_DT_ID")
                                End If
                                If Not IsDBNull(drScheduleTime.Item("PRDC_FLTR_ID")) Then
                                    i32PRDC_FLTR_ID = drScheduleTime.Item("PRDC_FLTR_ID")
                                End If
                                dtLstRunUpdate = System.DateTime.Now.Date & " " & drScheduleTime.Item("GNRTN_TM").ToString
                                Dim dtNextDate As Date = pub_getNextRunDateTime(i32PRDC_FLTR_ID, drScheduleTime.Item("GNRTN_TM").ToString, i32PRDC_DT)
                                dtNextDate = dtNextDate.Date & " " & drScheduleTime.Item("GNRTN_TM").ToString
                                objDAR.pub_UpdateNextRunDate(drScheduleTime.Item(DARData.DPT_ID), drScheduleTime.Item(DARData.CSTMR_ID), drScheduleTime.Item(DARData.RPRT_ID), dtNextDate, dtLstRunUpdate, drScheduleTime.Item(DARData.CSTMR_EML_STTNG_BIN))
                            End If
                            ''Ends here
                            For Each drEmailService As DataRow In dsEmailSetting.Tables(DARData._V_CUSTOMER_EMAIL_SETTING).Select(DARData.CSTMR_ID & "=" & drScheduleTime.Item(DARData.CSTMR_ID))
                                If drScheduleTime.Item(DARData.CSTMR_ID).ToString <> "" Then
                                    pvt_Diagnostic("Customer: " & drScheduleTime.Item(DARData.CSTMR_ID))
                                End If
                                ...
                                If i32PRDC_FLTR_ID = 33 Then
                                    ...
                                    blnGnrtnday = "True"
                                    pvt_Diagnostic("Type: " & i32PRDC_FLTR_ID & " Next Run Date: " & dtNextRnDate)
                                ...
                                ElseIf i32PRDC_FLTR_ID = 35 Then
                                    If i32PRDC_DY = 37 And Today.DayOfWeek = 0 Then
                                        dtNextRnDate = Now.Date & " " & strGNRTN_TM
                                        blnGnrtnday = "True"
                                        pvt_Diagnostic("Periodic Day: " & i32PRDC_DY & " Next Run Date: " & dtNextRnDate)
...
#Region "pvt_GetDAR"
    Private Sub pvt_GetDAR(ByRef br_drCustomerEmailSetting As DataRow)
        Try
            ...
            If dsDepot.Tables(DepotData._V_DEPOT).Rows.Count > 0 Then
            For depotID As Integer = 0 To dsDepot.Tables(DepotData._V_DEPOT).Rows.Count - 1
                Dim strParam As String = String.Concat("Customer='", i64CustomerID, "'&Next Test Type='51','52'&Out Date From=", Now.AddDays(-2).ToString("dd-MMM-yyyy"), "&Out Date To=", Now.ToString("dd-MMM-yyyy"))
                    dsCommon = objCommonUI.GetStatusReport(strParam, CInt(dsDepot.Tables(DepotData._V_DEPOT).Rows(depotID)(DepotData.DPT_ID)), _
                                                   "Inventory", 101) 'i64DepotID
                    pvt_GenerateReport(strstatusRDLCName, dsCommon, _
                                       br_drCustomerEmailSetting, "Landscape", dsDepot.Tables(DepotData._V_DEPOT).Rows(depotID)(DepotData.DPT_CD))
            Next
            End If
            dtNxtRunUpdate = pub_getNextRunDateTime(i32PRDC_FLTR_ID, strGNRTN_TM, i32PRDC_DT)

            'Added by Arun 2/2/2016
            dtLstRunUpdate = System.DateTime.Now.Date & " " & strGNRTN_TM
            ...
            objDAR.pub_UpdateCustomerEmailSetting(i64DepotID, i64CustomerID, intRprtId, dtNxtRunUpdate, dtLstRunUpdate, cstmr_email_id)
...
#Region "Generate Report"
    ...
    Private Sub pvt_GenerateReport(ByVal bv_strRDLCName As String, ByRef br_dsReport As DataSet, ByRef br_drCustomerEmailSetting As DataRow, ByVal bv_reportFormat As String, ByVal bv_DepotCode As String)
        Try
            ...
            pvt_Diagnostic("Excel File Generated End..")
            While dataToRead > 0
            ...
            If Not br_drCustomerEmailSetting.Item(DARData.SBJCT_VCR) Is DBNull.Value Then
                strSubject = br_drCustomerEmailSetting.Item(DARData.SBJCT_VCR) + " For Depot: " + bv_DepotCode
            Else
