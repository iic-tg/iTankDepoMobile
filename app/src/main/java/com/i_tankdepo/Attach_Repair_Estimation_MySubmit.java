...
* */
public class Attach_Repair_Estimation_MySubmit extends CommonActivity implements View.OnClickListener {
    ...
    private TextView tv_amount,tv_totalCost;
    private ImageView add_pen,im_skip;
    private String add_detail_jsonobject;
    private String In_date,customer_Id;
...
//                notification_text.setText((GlobalConstants.attach_count));
        ...
        if(GlobalConstants.from.equalsIgnoreCase("Repairpending_delete"))
        {
            try {
                encodeArray= GlobalConstants.multiple_encodeArray;
                adapter = new UserListAdapter(Attach_Repair_Estimation_MySubmit.this, R.layout.image_list_row, encodeArray);
     ...
     /*   if(GlobalConstants.from.equalsIgnoreCase("MySubmitRepair")|| GlobalConstants.from.equalsIgnoreCase("MySubmitRepair_Summary")
          ...
          *//*  //   repair_arraylist=GlobalConstants.repair_arraylist;
            ...
            if(GlobalConstants.summaryfrom.equalsIgnoreCase("summaryfrom"))
            {
                try {
                    if (encodeArray.size() > 0) {
                        adapter = new UserListAdapter(Attach_Repair_Estimation.this, R.layout.image_list_row, encodeArray);
                        attachment_list.setAdapter(adapter);
                    }
                }catch (Exception e)
                {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.im_capture:
                ...
                AlertDialog.Builder builder = new AlertDialog.Builder(Attach_Repair_Estimation_MySubmit.this);
                builder.setTitle("Add Photo!");
                builder.setItems(items, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int item) {
                        boolean result= Utility.checkPermission(Attach_Repair_Estimation_MySubmit.this, isCamPermission);
                        ...
                        if (items[item].equals("Take Photo")) {
                            ...
                            if(result)
                                startActivityForResult(new Intent(Attach_Repair_Estimation_MySubmit.this, CustomGallery_Activity.class), CustomGallerySelectId);
            ...
            case R.id.Line_iteam:
                ...
                try {
                    for (int i = 0; i < encodeArray.size(); i++) {
                        ...
                        invitejsonObject.put("base64imageString", encodeArray.get(i).getImage());
            ...
            case R.id.add_details:
                ...
                GlobalConstants.attach_count= String.valueOf(encodeArray.size());
                GlobalConstants.multiple_encodeArray=encodeArray;
                 invoice_id=GlobalConstants.invoice_PartyID;
                GlobalConstants.invoice_PartyID= invoice_id;
                GlobalConstants.summaryfrom="summaryfrom";
                GlobalConstants.Line_item_Json=Line_item_Json;
...
//            imagesPathList.add(imagesArray(i));
            ...
            byte[] byteArrayImage = byteArrayOutputStream.toByteArray();
                ...
                image_bean.setUriPath("");
                image_bean.setAttachment_Id("");
                image_bean.setImage("image_"+imagesArray.get(i));
                encodeArray.add(image_bean);
...
/*
                                ...
                                {
                                    final String fileName = list.get(i).getImage_Name().substring(list.get(i).getImage_Name().lastIndexOf('/') + 1);
                                    if(! fileName_from.equals(fileName))
                                    {
                                        image_bean = new Image_Bean();


                                        image_bean.setImage_Name(list.get(i).getImage_Name());
                                        image_bean.setImage(list.get(i).getImage());
                                        image_bean.setUriPath(list.get(i).getUriPath());
                                        image_bean.setAttachment_Id(list.get(i).getAttachment_Id());
                                        encodeArray.add(image_bean);
                                    }
                                }
*/
    ...
    public class Get_Repair_MySubmit_details extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            progressDialog = new ProgressDialog(Attach_Repair_Estimation_MySubmit.this);
